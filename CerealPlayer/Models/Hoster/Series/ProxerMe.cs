...
namespace CerealPlayer.Models.Hoster.Series
{
    public class ProxerMe : IVideoHoster
    {
        ...
        public EpisodeInfo GetInfo(string website)
        {
            ...
            var source = System.Threading.Tasks.Task
                .Run(async () => { return await models.Web.Html.GetAsynch(website); }).Result;
            ...
            if (!int.TryParse(addressParts[addressParts.Length - 2], NumberStyles.Integer, culture,
                out var episodeNum))
            {
                episodeNum = 0;
            }
        ...
        private abstract class RevealHoster : ISubTask
        {
            ...
            public async void Start()
            {
                try
                {
                    ...
                    // search for streams =
                    var res = JsonConvert.DeserializeObject<StreamInfo[]>(jsArray);
